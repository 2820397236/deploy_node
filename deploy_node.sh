...
ssr_url="https://github.com/cndf/shadowsocksr.git"
...
sed -i '$a\* * * * 1 root ntpdate cn.pool.ntp.org >> /dev/null 2>&1' /etc/crontab
...
install_caddy() {
	...
	if [[ $sys_bit == "x86_64" ]]; then
		local caddy_download_link="https://caddyserver.com/api/download?os=darwin&arch=amd64&idempotency=3347515095667"
	else
